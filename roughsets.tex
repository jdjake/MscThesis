...
\begin{remarks}
	\
	\begin{enumerate}
		\item[1.] When $s < d$, avoiding the configuration $\C$ is trivial, since the set $X = [0,1]^d - \pi(\C)$ is full dimensional and solves the pattern avoidance problem, where $\pi(x_1, \dots, x_n) = x_1$ is a projection map from $\C^n(\RR^d)$ to $\RR^d$. We will therefore assume that $s \geq d$ in our proof of the theorem. Note that obtaining a full dimensional set in the case $s = d$, however, is still interesting.
...
\begin{lemma} \label{discretelemma}
	Fix $k$, $1 \leq s < dn$, and $0 \leq \varepsilon < (dn - s)/2$. Let $T \subset \RR^d$ be a nonempty, $\DQ_k$ discretized set, and let $B \subset \RR^{dn}$ be a nonempty $\DQ_{k+1}$ discretized set such that
	%
	\[ \#(\DQ_{k+1}(B)) \leq N_{k+1}^{s + \varepsilon}. \]
	%
	Then there exists a constant $C(s,d,n)$\footnote{The proof will show that we can choose $C(s,d,n) \sim_d 4^{\frac{1}{dn - s}}$, which explodes as $s \to dn$.}, depending only on the constants $s$, $d$, and $n$, such that, provided
	...
	%
	\begin{enumerate}
		\item\label{avoidanceItem} \emph{Avoidance}: For any collection of $n$ distinct cubes $Q_1, \dots, Q_n \in \DQ_{k+1}(S)$, $Q_1 \times \dots \times Q_n \not \in \DQ_{k+1}(B)$.

		\item\label{nonConcentrationItem} \emph{Sparse Regularity}: For each $Q \in \DQ_k(T)$, there exists $\mathcal{R}_Q \subset \DR_{k+1}(Q)$ such that $\#(\mathcal{R}_Q) \geq \#(\DR_{k+1}(Q))$, and if $R \in \DR_{k+1}(Q)$,
		%
		\[ \#(\DQ_{k+1}(R)) = \begin{cases} 1 & : R \in \mathcal{R}_Q \\ 0 & : R \not \in \mathcal{R}_Q. \end{cases} \]
	\end{enumerate}
\end{lemma}
...
\begin{proof}
	...
	\end{equation}
	%
	where $\pi \colon \RR^{dn} \to \RR^d$ is the projection map $(x_1, \dots, x_n) \mapsto x_1$, for $x_i \in \RR^d$. For any distinct $R_1, R_2 \in \DR_{k+1}(T)$, $Q_{R_1} \cap Q_{R_2} = \emptyset$, since $Q_{R_1} \subset R_1^\circ$, $Q_{R_2} \subset R_2^\circ$, and $R_1^\circ \cap R_2^\circ = \emptyset$. Given any strongly non-diagonal cube $K = K_1 \times \cdots \times K_n \in \DQ_{k+1}(B)$, either $K \not \in \DQ_{k+1}(A^n)$, or $K \in \DQ_{k+1}(A^n)$. If the former occurs then $K \not \in \DQ_{k+1}(S(A))$ since $S(A) \subset A$, so $\DQ_{k+1}(S(A)^n) \subset \DQ_{k+1}(A^n)$. If the latter occurs then $K \in \mathcal{K}(A)$, and since $\pi(K) = K_1$, $K_1 \not \in \DQ_{k+1}(S(A))$. In either case, $K \not \in \DQ_{k+1}(S(A)^n)$, so $S(A)$ satisfies Property \ref{avoidanceItem}. By construction, $A$ contains at most one sub cube $Q \in \DQ_{k+1}(R)$ for each $R \in \DR_{k+1}(T)$. Since $S(A) \subset A$, $S(A)$ satisfies Property \ref{nonConcentrationItem}. Thus the set $S(A)$ satisfies Property \ref{avoidanceItem}, regardless of which values are assumed by the random variables $\{ Q_R \}$. Next we will show that with non-zero probability, the set $S(A)$ satisfies Property \ref{nonConcentrationItem}. 

	For each cube $Q \in \DQ_{k+1}(T)$, there is a unique `parent' cube $R \in \DR_{k+1}(T)$ such that $Q \subset R$. Note that \eqref{rBound} implies, if $C(s,d,n) \geq 4d$, that $N_{k+1} \geq 4d M_{k+1}$. Since $Q_R$ is chosen uniformly from $\DQ_{k+1}(R)$, for any $Q \in \DQ_{k+1}(R)$,
	%
	\[ \prob(Q \subset A) = \prob(Q_R = Q) = \left( \DQ_{k+1}(R) \right)^{-1} = (M_{k+1}/N_{k+1})^d. \]
	%
	The cubes $\{ Q_R \}$ are chosen independently, so if $K_1, \dots, K_n$ are distinct cubes in $\DQ_{k+1}(T)$, then either the cubes $K_1, \dots, K_n$ have distinct parents, in which case we apply the independence of the random cubes $\{ Q_R \}$ to conclude that
	%
	\[ \prob(K_1, \dots, K_n \subset A) \leq (M_{k+1}/N_{k+1})^{dn}. \]
	%
	If the cubes $K_1, \dots, K_n$ do not have distinct parents, Property \ref{nonConcentrationItem} implies
	%
	...
	\end{equation}
	%
	In other words, $S(A_0) \subset A_0$ is obtained by removing at most $(1/2) \cdot M_{k+1}^d$ cubes in $\B^d_s$ from $A_0$. For each $Q \in \B_l^d(T)$, we know that $\# \DQ_{k+1}(Q \cap U_0) = M_{k+1}^d$. Combining this with \eqref{KU0Small}, we arrive at the estimate 
	%
	\begin{align*}
...
\begin{lemma} \label{stronglydiagonal}
	Let $\{ l_k \}$ be a sequence of positive numbers converging to zero, and let $B \subset \RR^{dn}$. Let $\{ B_k \}$ be a sequence of sets, with each $B_k$ an $l_k$ discretized subset of $\RR^{dn}$, such that the interiors $\{ B_k^\circ \}$ strongly cover $B$. For each index $k$, let $X_k$ be an $l_k$ discretized subset of $\RR^d$. Suppose that for each $k$, $\DQ_k(X_k^n) \cap \DQ_k(B_k)$ contains no strongly non diagonal cubes. If $X = \bigcap X_k$, then for any distinct $x_1, \dots, x_n \in X$, we have $(x_1, \dots, x_n) \not \in B$.
\end{lemma}
\begin{proof}
	Let $x = (x_1, \dots, x_n) \in B$ be a point such that $x_1, \dots, x_n$ are distinct. Define
	%
	\[ \Delta = \{ (w_1, \dots, w_n) \in \RR^{dn} \setcolon \text{there exists $i \neq j$ such that $w_i = w_j$} \}. \]
	%
	Then $d(\Delta,x) > 0$, where $d$ is the Hausdorff distance between $\Delta$ and $x$. Since $\{ B_k \}$ strongly covers $B$, there is a subsequence $\{ k_m \}$ such that $x \in B_{k_m}^\circ$ for every index $m$. Since $l_k$ converges to 0 and thus $l_{k_m}$ converges to $0$, if $m$ is sufficiently large then $\sqrt{dn} \cdot l_{k_m} < d(\Delta,x)$. Note that $\sqrt{dn} \cdot l_{k_m}$ is the diameter of a cube in $\DQ_{k_m}$. For such a choice of $m$, any cube $Q \in \B^d_{l_{k_m}}$ which contains $x$ is strongly non-diagonal. Thus $x \in B_{k_m}^\circ$. Since $X_{k_m}$ and $B_{k_m}$ share no cube which contains $x$, this implies $x \not \in X_{k_m}$. In particular, this means $x \not \in X^n$.
\end{proof}
...
%
Then $1 \leq M_{k+1} \leq N_{k+1}$, and \eqref{rBound} is satisfied. Setting $B = B_{k+1}$, and $T = X_k$. Then we can apply Lemma \ref{discretelemma} to find a set $S$ satisfying all Properties of that Lemma. We then set $X_{k+1} = S$.
...
%
\begin{equation} \label{equation1403463468957983} N_k \lesssim M_k^{1/t} \end{equation}
%
where $t = (nd - s)/d(n-1)$. \eqref{equation1403463468957983} is a form of Property \ref{ChangeofScales} of Theorem \ref{TheConstructionTheorem}. Thus all assumptions of Theorem \ref{TheConstructionTheorem} are satisfied, and so we find $X$ has Hausdorff dimension $(nd - s)/(n-1)$.
...
\begin{theorem} \label{sumset-application} 
	Let $Y \subset \RR^d$ be a countable union of sets of Minkowski dimension at most $t < d$. Then there exists a set $X \subset \RR^d$ with Hausdorff dimension at least $d - t$ such that $X + X$ is disjoint from $Y$.
\end{theorem}
\begin{proof}
