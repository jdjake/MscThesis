...
\begin{example}[Linear Independence Configuration]
	...
	%
	\[ \C = \bigcup_{n = 1}^\infty \{ (x_1, \dots, x_n) \in \C^n(V): \text{for any}\ a_1, \dots, a_n \in K,\ a_1x_1 + \dots + a_nx_n = 0 \}. \]
	%
...
\begin{itemize}
	...
	%
	We have no need for this type of thickening in this thesis.
\end{itemize}
%
Since any cube is covered by at most $O_d(1)$ cubes in $\DD$ of comparable sidelength, from the perspective of measure theory, working with dyadic cubes is normally sufficient. For instance,

Our main purpose with working with dyadic cubes is to construct \emph{fractal-type sets}. By this, we mean defining sets $X$ as the intersection of a nested family of sets $\{ X_k \}$, where each $X_k$ is $\DD_k$ discretized, and each succesive set $X_{k+1}$ is obtained from $X_k$ by repeatedly performing a simple, recursive procedure. Such a construction satisfies Property (i), (ii), and (v) of Falconer's definition of a fractal, justifying the name.
...
\begin{lemma}
	...
	%
	\begin{itemize}
		\item \emph{Separation}: For each $k$, the collection $\{ \overline{A} : A \in \mathcal{E}_k \}$ is disjoint.
