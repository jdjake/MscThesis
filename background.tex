...
\begin{example}[Isosceles Triangle Configuration]
	...
	%
	A set $X$ avoids isosceles triangles if and only if $\C^3(X)$ is disjoint from $\C$. %Notice that $|x_1 - x_2| = |x_1 - x_3|$ holds if and only if $|x_1 - x_2|^2 = |x_1 - x_3|^2$, which is an algebraic equation in the coordinates of $x_1,x_2$, and $x_3$. Thus $\C$ is an algebraic hypersurface of degree two in $\RR^6$.
\end{example}
